...
public class Volo {
	...
	void aggiungiPasseggero(Passeggero p) throws ExceedingPassengersCapacityException {
		if (p.getClasse() == 'E' && postiDispE > 0) {
			...
			throw new ExceedingPassengersCapacityException(this, p.getClasse());
	...
	/**
	 * Deve dichiarare di poter lanciare eccezione, in quanto è eccezione
	 * controllata (non Runtime)
	 */
	void rimuoviPasseggero(int idBiglietto) throws Exception {
		...
		for (int i = 0; i < passeggeri.size(); i++) {
			...
			if (indice != -1) {
				...
				return; // passeggero rimosso
			}
		}	
		throw new PassengerNotFoundException(this, idBiglietto);
	...
	void decolla() throws NoPassengersException {
		if (passeggeri == null || passeggeri.isEmpty())
			throw new NoPassengersException(this);
		else
			System.out.println("Sto decollando..." + toString());
	}
}
